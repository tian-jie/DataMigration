...
//#endif
            ...
            {
                if (!string.IsNullOrEmpty(import) && !import.StartsWith("#"))
                {
                    var task = typeof(Program).GetMethod("Import").MakeGenericMethod(modelAssembly.GetType(import, true, true)).Invoke(null, null);
                }
            }
        ...
        public static void Import<T>() where T : IEntity, new()
        {
            ...
            try
            {
                ...
                for (var i = 0; ; i++)
                {
                    ...
                    Console.WriteLine($"  start to import {tableName}.{i.ToString("D4")} to mssql database... - {watch.ElapsedMilliseconds / 1000.0}s");

                    db.BulkInsert<T>(result, options => options.InsertKeepIdentity = true);
                    //db.AddRange(result);
                    //Console.WriteLine($"  data added to {tableName}.{i.ToString("D4")} to mssql database... - {watch.ElapsedMilliseconds / 1000.0}s");
                    //db.BulkSaveChanges();
                    Console.WriteLine($"  finished to import {tableName}.{i.ToString("D4")} to mssql database... - {watch.ElapsedMilliseconds / 1000.0}s");
                }
